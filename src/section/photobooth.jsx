...
function Photobooth() {
    ...
    const startCaptureSequence = () => {
        ...
        let count = 3;
        let capturedCount = 0; // Track captured images

        const capturePhotos = () => {
            if (count === 0 && capturedCount < 3) {
                const screenshot = webcamRef.current.getScreenshot();
                setImages((prev) => [...prev, screenshot]); // Add image immediately
                capturedCount++;

                if (capturedCount < 3) { // Continue until 3 images are captured
                    count = 3; // Reset countdown
                    setCountdown(3);
                    setTimeout(capturePhotos, 1000);
                } else {
                    setIsCapturing(false);
                    setCountdown(null); // Stop displaying countdown
                }
                return;
            }
